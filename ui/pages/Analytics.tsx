      ...
      {analytics ? (
        <ul className="grid grid-flow-row gap-8 grid-col-1 grid-cols-2 auto-rows-auto">
          {analytics.data?.map(({ label, pageViews, uniques, pages, returning }) => (
            ...
            >
              <h2 className="text-2xl mb-2 bg-gray-900 text-white py-4 px-6 shadow-sm">
                {label}
              </h2>
              ...
              <table className="m-4 break-all" style={{ width: 'calc(100% - 2rem)' }}>
                <thead>
                  <tr>
                    <th> </th>
                    <th className="font-light text-xs uppercase text-right pr-3">
                      Views
                    </th>
                    <th className="font-light text-xs uppercase text-right pr-3">
                      Uniques
                ...
                <tbody>
                  {pages.map(({ path, pageViews, uniques }, index) => (
                    <tr key={path} className={index % 2 === 0 ? '' : 'bg-gray-200'}>
                      ...
                      <td
                        className="p-1 px-3 text-right align-top text-sm text-gray-700"
                        width="10%"
                      >
                        {num(pageViews)}
                      </td>
