...
const createUrlParamsHook = <U extends Record<string, SupportedTypesInUrl>>(paramTypeDefinition: U) => (
  <T extends string | number | boolean>(paramName: keyof typeof paramTypeDefinition) => {
    ...
    const setParamValue = (paramValue: T) => history.replace({ search: updateQueryString(paramName as string, String(paramValue)) });
    return [paramValue, setParamValue] as [T | undefined, (x: T) => void];
  }
);
