...
const Developer2: React.FC<DeveloperProps> = ({ dev, isFirst, queryPeriodDays }) => {
    ...
    >
      ...
      >
        <div className="flex gap-3">
          ...
          />
          <div>
            <h3 className="font-bold text-md">{dev.name}</h3>
            <div className="text-base font-normal text-gray-600">
              <span className="text-blue-gray text-sm my-2">
                <span className="font-semibold text-base">{commitsCount}</span>
                {` ${commitsCount === 1 ? 'commit' : 'commits'} in `}
                <span className="font-semibold text-base">{dev.repos.length}</span>
