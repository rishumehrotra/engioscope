...
const totalWorkCenterTimeInQueryPeriodUsing = (
  ...
  const workCenterTimeInQueryPeriod = workCenterTimeInQueryPeriodUsing(
    workItemTimes,
    queryPeriodDays
  );
  return (workItems: UIWorkItem[]) =>
    workItems.reduce(
      (acc, workItemId) => acc + workCenterTimeInQueryPeriod(workItemId),
      0
    );
};
...
const EffortDistributionGraph: React.FC<EffortDistributionProps> = ({
  ...
  const workItemsToDisplay = useMemo(
        ...
        Object.entries(organizeByWorkItemType(preFilteredWorkItems, filter)).filter(
          ([, group]) =>
            Object.values(group).reduce(
              (acc, wis) => acc + totalWorkCenterTimeInQueryPeriod(wis),
              0
            ) > 0
        )
      ),
    [
      ...
      filter,
      totalWorkCenterTimeInQueryPeriod,
    ]
  );
  ...
  const effortDistributionStringifiedForWit = useCallback(
    (x: unknown, witId: string) => {
      ...
      const matchingEntries = Object.entries(effortDistribution.byGroup).filter(([key]) =>
  ...
  const effortDistributionStringifiedForGroup = useCallback(
    (x: unknown, witId: string, groupName: string) =>
      effortDistribution.total
        ? `${(
            (effortDistribution.byGroup[witId + groupName] * 100) /
            effortDistribution.total
          ).toFixed(2)}%`
        : '-',
    [effortDistribution.byGroup, effortDistribution.total]
  ...
  const effortDistributionSplitup = useCallback(
    (workItem: UIWorkItem) => (
      <div className="text-gray-500 text-sm ml-6 mb-2">
        {workItemTimes(workItem)
          .workCenters.map(wc => {
            if (!wc.end || new Date(wc.end) < queryPeriodStart) {
              return null;
            }
            if (new Date(wc.start) > queryPeriodStart) {
              return wc;
            }
            return {
              ...wc,
          ...
          .map(
            wc =>
              `${wc.label} time ${prettyMS(timeDifference(wc))} (${shortDate(
                new Date(wc.start)
  ...
  const legendSidebarProps = useMemo<LegendSidebarProps>(() => {
    ...
    const items = getSidebarItemStats(
      workItemsToDisplay,
      accessors,
      effortDistributionStringifiedForGroup
    );

    const headlineStats = getSidebarHeadlineStats(
      workItemsToDisplay,
      workItemType,
      effortDistributionStringifiedForWit,
      ''
    );
    ...
    effortDistributionStringifiedForWit,
    openModal,
    workCenterTimeInQueryPeriod,
    workItemTooltip,
    effortDistributionSplitup,
  ]);
              ...
              <li key={witId}>
                <details>
                  <summary className="cursor-pointer">
                    {`Effort for ${workItemType(
                      witId
                    ).name[1].toLowerCase()} is the time spent in `}
                    {`${listFormat(
                      workItemType(witId).workCenters.map(wc => wc.label)
                    )}.`}
                  </summary>
