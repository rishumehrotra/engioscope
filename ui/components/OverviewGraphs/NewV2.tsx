...
const dataByDay = (
  ...
  const lines: Map<
    ...
    Map<string, number>
  > = new Map(
    Object.entries(workItemSummary).flatMap(([workItemTypeName, groups]) =>
      Object.entries(groups).map(([groupName, dates]) => [
        { workItemType: workItemTypeName, groupName },
        new Map(Object.entries(dates)),
      ])
    )
  );

  return [...lines.entries()].map(
    ([{ workItemType, groupName }, dates]): Line => ({
      label:
        groupName === noGroup
...
const useSidebarCheckboxState = () => {
  ...
  const isChecked = useCallback(
    (key: string) => Boolean(checkboxState[key]),
    [checkboxState]
  );
  ...
  const setSummary = useCallback(
    (workItemSummary: Record<string, Record<string, Record<string, number>>>) => {
      setCheckboxState(() =>
        Object.entries(workItemSummary).reduce<Record<string, boolean>>(
          (acc, [workItemTypeName, groups]) => {
...
const NewWorkItemsByGroup: React.FC<NewWorkItemsByGroupProps> = ({
  workItemTypeName,
  groupName,
}) => {
...
const NewWorkItemsByDate: React.FC<NewWorkItemsByDateProps> = ({ date }) => {
  ...
  const workItems = trpc.workItems.nenwWorkItemsListForDate.useQuery({
    ...
    additionalFilters,
    date,
    timeZone: qp.queryPeriod[2],
  });

  if (!workItems.data) return <Loading />;

  return <WorkItemsByGroup workItems={workItems.data} />;
};
...
const NewGraph: React.FC<NewGraphProps> = ({ openModal }) => {
  ...
  const dataToShow = useMemo(
    () =>
      dataByDay(
        workItemSummary.data || {},
        workItemTypes.data || [],
        queryPeriodDays
      ).filter(line =>
        isChecked(
          `${line.workItemTypeName}${
  ...
  const legendSidebarProps = useMemo(
    (): LegendSidebarProps => ({
      headlineStats: Object.entries(workItemSummary.data || {}).map(
        ([workItemTypeName, groups]) => ({
          label:
            workItemTypes.data?.find(wit => wit.name[0] === workItemTypeName)?.name[1] ||
            workItemTypeName,
          value: num(sum(Object.values(groups).flatMap(group => Object.values(group)))),
          unit: '',
        })
      ),
      items: Object.entries(workItemSummary.data || {}).flatMap(
        ([workItemTypeName, groups]) => {
          const matchingWit = workItemTypes.data?.find(
      ...
      onItemClick: key => {
        const wit = (workItemTypes.data || []).find(
          wit => key.startsWith(`${wit.name[0]}-`) || key === wit.name[0]
        );

        if (!wit) return;

        const groupName =
          key === wit.name[0] ? noGroup : key.replace(`${wit.name[0]}-`, '');
        const groupCount = sum(
          Object.values(workItemSummary.data?.[wit.name[0]][groupName] || {})
        );

        return openModal({
          heading: 'New work items',
          subheading: `${wit.name[1]} ${
            groupName === noGroup ? '' : `/ ${groupName}`
          } (${groupCount})`,
          body: (
            <NewWorkItemsByGroup workItemTypeName={wit.name[0]} groupName={groupName} />
          ),
        });
      },
      onCheckboxClick,
      ...
      <CrosshairBubble
        ...
        items={dataToShow
          .map(line => {
            ...
            const matchingWit = (workItemTypes.data || []).find(
              wit => wit.name[0] === line.workItemTypeName
            )!;
  ...
  const lineGraphProps = useMemo<LineGraphProps<Line, Point>>(
    () => ({
      ...
      lineColor: line =>
        lineColor({ groupName: line.groupName, witId: line.workItemTypeName }),
      ...
      left={
        <>
          ...
          <ul className="text-sm text-gray-600 pl-8 mt-4 list-disc bg-gray-50 p-2 border-gray-200 border-2 rounded-md">
            {(workItemTypes.data || [])
              .filter(wit =>
                Object.keys(workItemSummary.data || {}).includes(wit.name[0])
              )
              .map(wit => (
                <li key={wit.name[0]}>
