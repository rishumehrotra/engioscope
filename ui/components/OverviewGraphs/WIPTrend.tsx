import { allPass, always, last, length, pipe, prop, T } from 'rambda';
...
const WIPTrendGraph: React.FC<WIPTrendGraphProps> = ({
  ...
  const { organizeByWorkItemType, workItemType, queryPeriodDays, groupLabel } = accessors;
  ...
  const hasData = useMemo(
    () =>
      splitByDateForLineGraph(
        ...
        (acc, line) => acc + length(line.workItemPoints.flatMap(p => p.workItems)),
        0
      ) > 0,
  ...
  const csvData = useMemo(
    () => wipWorkItemsCSV(workItems, accessors),
    [workItems, accessors]
  );
      ...
      (_, witId) =>
        num(
          dataByDay
            ...
            // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
            .flatMap(item => last(item.workItemPoints)!.workItems).length
        ),
      'today'
    );
          ...
          <ul className="text-sm text-gray-600 pl-8 mt-4 list-disc bg-gray-50 p-2 border-gray-200 border-2 rounded-md">
            {Object.keys(workItemsToDisplay).map(witId => (
              <li key={witId}>
                {`A ${workItemType(
                  witId
                ).name[0].toLowerCase()} is considered to be WIP if it has a `}
                {/* eslint-disable-next-line @typescript-eslint/no-non-null-assertion */}
                {`${stringifyDateField(workItemType(witId).startDateFields!).replace(
                  ', whichever is earlier',
                  ''
                )}, but doesn't have a `}
                {/* eslint-disable-next-line @typescript-eslint/no-non-null-assertion */}
                {`${stringifyDateField(workItemType(witId).endDateFields!).replace(
                  ', whichever is earlier',
                  ''
                )}.`}
              </li>
