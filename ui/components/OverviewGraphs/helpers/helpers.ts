...
export const lineColor = (() => {
  ...
  return ({ witId, groupName }: GroupLabel) => c(witId + groupName);
})();
...
export const timeSpent = (workItemType: UIWorkItemType) => (times: WorkItemTimes) =>
  times.start
    ? workItemType.workCenters
        .reduce<TimeInArea[]>((acc, wc, index) => {
          ...
          acc.push(
            {
              ...
              start: index === 0 ? new Date(times.start!) : last(acc)!.end!,
...
export const workItemAccessors = (
  projectAnalysis: ProjectOverviewAnalysis,
  queryPeriodDays: number
) => {
  const { overview, lastUpdated, ignoreForWIP, environments } = projectAnalysis;
  ...
  return {
    ...
    wasWorkItemOpenedInLastThreeMonths: isNewInTimeRange(
      workItemType,
      workItemTimes
    )(d => d > startOfQueryPeriod),
    organizeByWorkItemType: (
      workItems: UIWorkItem[],
      filter: (wi: UIWorkItem) => boolean
    ) =>
...
export const getSidebarHeadlineStats = (
  ...
  Object.entries(organizedWorkItems).map(([witId, groups]) => ({
...
export const getSidebarItemStats = (
  ...
  Object.entries(organizedWorkIItems).reduce<LegendSidebarProps['items']>(
    (acc, [witId, groups]) => {
...
export const useSidebarCheckboxState = (organizedAllWorkItems: OrganizedWorkItems) => {
  const [checked, setChecked] = useState(
    Object.entries(organizedAllWorkItems).reduce<Record<string, boolean>>(
      (acc, [witId, groups]) => {
...
export const getSidebarStatByKey = (
  key: string,
  organizedWorkItems: OrganizedWorkItems
) => {
  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  const [matchingWitId, groups] = Object.entries(organizedWorkItems).find(([witId]) =>
    key.startsWith(witId)
  )!;

  // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
  const [matchingGroup, workItems] = Object.entries(groups).find(
    ([groupName]) => key === matchingWitId + groupName
  )!;
...
export const workItemStateUsing =
  ({ workItemTimes }: WorkItemAccessors, wit: UIWorkItemType) =>
  (workItem: UIWorkItem) => {
