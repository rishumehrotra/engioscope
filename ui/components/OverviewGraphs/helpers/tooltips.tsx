...
const addSection = (section: TooltipSection) => `
  <div class="${section.width === 2 ? 'col-span-2' : ''}">
    ...
    <div class="rounded-md bg-gray-500 mt-1 h-1.5 w-full">
      <div class="bg-gray-300 h-1.5 rounded-md" style="width: ${
        section.graphValue * 100
      }%"></div>
    </div>`
        : ''
    }
...
const standardSections = (
  ...
  workItemGroupName
    ? { label: workItemType.groupLabel || 'Group', value: workItemGroupName }
    : undefined,
  workItem.priority
    ? {
        label: 'Priority',
  ...
  workItem.rca.length
    ? { label: 'RCA', value: workItem.rca.join(' / '), width: 2 }
    : undefined,
];
...
export const createCompletedWorkItemTooltip =
  ...
  (workItem: UIWorkItem, additionalSections: TooltipSection[] = []) => {
    ...
    const sections: (TooltipSection | undefined)[] = [
      ...
      clt
        ? {
            ...
            graphValue: divide(clt, ct).getOr(undefined),
          }
        : undefined,
      worstOffender
        ? {
...
export const createWIPWorkItemTooltip =
  ({ workItemType, workItemTimes, workItemGroup }: WorkItemAccessors) =>
  (workItem: UIWorkItem, additionalSections: TooltipSection[] = []) => {
    const worstOffender = head(
      computeTimes(workItemType(workItem.typeId), workItemTimes(workItem)).sort(
        desc(byNum(prop('timeDiff')))
      )
    );
    const wig = workItem.groupId ? workItemGroup(workItem.groupId) : undefined;
    ...
    const sections = [
      ...
      {
        label: 'Age',
        value: prettyMS(Date.now() - new Date(workItem.created.on).getTime()),
      },
      worstOffender
        ? {
            ...
            graphValue: divide(
              worstOffender.timeDiff,
              Date.now() - new Date(workItem.created.on).getTime()
              // eslint-disable-next-line unicorn/no-useless-undefined
            ).getOr(undefined),
          }
        : undefined,
