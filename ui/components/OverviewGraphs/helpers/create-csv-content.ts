...
export const closedWorkItemsCSV = (
  ...
  workItems.length === 0
    ...
    : [
        ...
        ...workItems.map(wi => {
          ...
          // eslint-disable-next-line @typescript-eslint/no-non-null-assertion
          const start = s!;
          const state = workItemStateUsing(
            accessors,
            accessors.workItemType(wi.typeId)
          )(wi);
          ...
          return [
            ...
            state.state,
            Math.round(
              (Date.now() - new Date(state.since).getTime()) / (1000 * 60 * 60 * 24)
            ),
            wi.priority || 'unknown',
            wi.url,
          ];
