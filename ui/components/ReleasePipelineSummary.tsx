  ...
  return (
    <ProjectStats>
      ...
      {(stagesToHighlight || []).map(stageName => {
        const stageExistsCount = count(incrementIf(pipelineHasStageNamed(stageName)))(
          pipelines
        );
        const stageExistsAndUsedCount = count(
          incrementIf(pipelineUsesStageNamed(stageName))
        )(pipelines);
              ...
              topStats={[
                {
                  ...
                  value: divide(stageExistsCount, pipelines.length)
                    .map(toPercentage)
                    .getOr('-'),
                ...
                },
                {
                  ...
                  value: divide(stageExistsAndUsedCount, pipelines.length)
                    .map(toPercentage)
                    .getOr('-'),
                  tooltip: `${num(stageExistsAndUsedCount)} out of ${
                    pipelines.length
                  } release ${
        ...
        topStats={[
          {
            ...
            value: (
              <LabelWithSparkline
                label={divide(masterDeploys.count, masterDeploys.total)
                  .map(toPercentage)
                  .getOr('-')}
            ...
            tooltip: `${num(masterDeploys.count)} out of ${num(
              masterDeploys.total
            )} releases were exclusively from master.${
              ignoreStagesBefore
                ? `<br />Branches that didn't go to ${ignoreStagesBefore} are not considered.`
                : ''
            }`,
          },
        ]}
        ...
        topStats={[
          {
            ...
            tooltip: `${num(policyPassCount)} out of ${num(
              pipelines.length
            )} have branches that conform to the branch policy.${
              ignoreStagesBefore
                ? `<br />Branches that didn't go to ${ignoreStagesBefore} are not considered.`
                : ''
            }`,
