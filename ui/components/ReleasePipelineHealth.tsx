...
const policyTooltip = (policy: NormalizedPolicies) => {
  ...
  const optionalTag = (key: keyof typeof policy) =>
    policy[key].isOptional ? '(optional)' : '';
  return `
    ...
    <ul class="w-72">
      <li>
        ...
        Minimum number of reviewers ${
          policy.minimumNumberOfReviewers.count === 0
            ? ''
            : `(${policy.minimumNumberOfReviewers.count})`
        }
  ...
  return (
    <div className="my-4">
      <div className="uppercase font-semibold text-sm text-gray-800 tracking-wide mb-2">
        Artifacts from repos
      </div>
      {Object.keys(pipeline.repos).length ? (
        <ol className="grid grid-flow-col justify-start">
          {Object.entries(pipeline.repos).map(
            ([repoId, { name, branches, additionalBranches }]) => (
              <div
        ...
        <div className="inline-flex bg-gray-100 py-3 px-4 rounded-lg">
          <AlertMessage message="No starting artifact found" type="info" />
        </div>
      )}
    </div>
  );
        ...
        <div className="mt-6">
          {formattedReleaseDefinition &&
            (formattedReleaseDefinition === 'loading' ? (
              <div className="text-gray-500 text-sm mb-10">Loading...</div>
            ) : (
