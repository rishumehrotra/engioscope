...
import { ArrowDown, ArrowUp, ExternalLink } from '../common/Icons.js';
...
const CollapsibleSection: React.FC<CollapsibleSectionProps> = ({ heading, table }) => {
  ...
  return (
    <details onToggle={onDetailsClick}>
      <summary className="font-semibold text-xl my-2 cursor-pointer">{heading}</summary>
        ...
        .map(group => {
          ...
          return {
            ...
            values: [
              ...
              {
                value: divide(
                  workItems.flowEfficiency.wcTime,
                  workItems.flowEfficiency.total
                ).getOr(0),
                content: renderMetric(
...
const QualityMetrics: React.FC<{
  ...
  const sections = useMemo(() => {
    ...
    const allEnvironments = [
      ...new Set(
        groups.flatMap(group => Object.keys(group.workItems[bugsDefinitionId] || {}))
      ),
    ].sort((a, b) => {
    ...
    return allEnvironments.reduce<{
      sections: (CollapsibleSectionProps & { key: string })[];
      encounteredEquivalentEnvironment: boolean;
    }>(
      ({ sections, encounteredEquivalentEnvironment }, env) => {
        const envDisplayName = equivalientEnvironments.includes(env)
          ? equivalientEnvironments.join(' or ')
          : env;

        if (equivalientEnvironments.includes(env) && encounteredEquivalentEnvironment) {
          return {
            sections,
            encounteredEquivalentEnvironment,
          };
        }
        ...
        sections.push({
          ...
          table: () => ({
            columns: [
              ...
              {
                label: 'New bugs',
                tooltip: `Number of bugs opened in the last ${queryPeriodDays} days`,
              },
              {
                label: 'Bugs fixed',
                tooltip: `Number of bugs closed in the last ${queryPeriodDays} days`,
              },
              {
                label: 'Bugs cycle time',
                tooltip: 'Average time taken to close a bug',
              },
              {
                label: 'Bugs CLT',
                tooltip: 'Average time taken to close a bug once development is complete',
              },
              {
                label: 'Flow efficiency',
                tooltip:
              ...
              {
                label: 'WIP trend',
                tooltip: `WIP bugs over the last ${queryPeriodDays} days`,
              },
            ...
            rows: groups.map(group => {
              ...
              const bugsForEnv = summaryBugsForEnv
                ? processSummary(summaryBugsForEnv)
                : null;
              ...
              return {
                ...
                values: [
                  ...
                  {
                    value: divide(
                      bugsForEnv?.flowEfficiency.wcTime || 0,
                      bugsForEnv?.flowEfficiency.total || 0
                    ).getOr(0),
                    content: renderBugMetric(
                  ...
                  },
                  {
                    ...
                    content: renderBugMetric(
                      bugsForEnv?.wipAge ? prettyMS(bugsForEnv.wipAge) : '-',
                      '#age-of-work-in-progress-items'
                    ),
                  },
                ],
        ...
        return {
          sections,
          encounteredEquivalentEnvironment: equivalientEnvironments.includes(env)
            ? true
            : encounteredEquivalentEnvironment,
        };
      },
...
const RepoAnalysisDetails: React.FC<{
  repoStats: SummaryMetrics['groups'][number]['repoStats'];
}> = ({ repoStats }) => (
...
const TestAutomationMetrics: React.FC<{ groups: SummaryMetrics['groups'] }> = ({
  groups,
}) => {
  const table: CollapsibleSectionProps['table'] = useCallback(
    () => ({
      columns: [
        ...
        {
          label: 'Tests',
          tooltip: 'Number of unit / components tests running in build pipelines',
        },
        { label: 'Coverage', tooltip: 'Percentage of code covered by tests' },
        ...groups[0].pipelineStats.stages.flatMap(stage => [
          {
            label: `Pipelines having ${stage.name}`,
            tooltip: `Percentage of pipelines having ${stage.name}`,
          },
          {
            label: `Pipelines using ${stage.name}`,
            tooltip: `Percentage of pipelines using ${stage.name}`,
          },
        ]),
      ],
      rows: groups.map(group => {
        ...
        return {
          ...
          values: [
            ...
            {
              value:
                repoStats.coverage === '-' ? 0 : Number.parseInt(repoStats.coverage, 10),
              content: reposMetric(
                <ExtendedLabelWithSparkline
            ...
            ...pipelineStats.stages.flatMap(stage => [
              ...
              },
              {
                ...
                content: pipelinesMetric(
                  divide(stage.used, pipelineStats.pipelines).map(toPercentage).getOr('-')
                ),
              },
            ]),
          ],
  ...
  return <CollapsibleSection heading="Test automation" table={table} />;
};

const CodeQualityMetrics: React.FC<{ groups: SummaryMetrics['groups'] }> = ({
  groups,
}) => {
  const table: CollapsibleSectionProps['table'] = useCallback(
    () => ({
      columns: [
        ...
        { label: 'Sonar', tooltip: 'Percentage of repos with Sonar configured' },
        {
          label: 'Ok',
          tooltip:
            'Percentage of pipelines with sonar configured that pass quality checks',
        },
        {
          label: 'Warn',
          tooltip:
            'Percentage of pipelines with sonar configured that have a warning for quality checks',
        },
        {
          label: 'Fail',
          tooltip:
            'Percentage of pipelines with sonar configured that fail quality checks',
        },
        {
          label: 'Branch policy met',
          tooltip: 'Percentage of pipelines conforming to branch policies',
        },
  ...
  return <CollapsibleSection heading="Code quality" table={table} />;
};
...
const CIBuilds: React.FC<CIBuildsProps> = ({ groups, queryPeriodDays }) => {
  const table: CollapsibleSectionProps['table'] = useCallback(
    () => ({
      columns: [
        ...
        {
          label: 'Runs',
          tooltip: `Number of CI builds run in the last ${queryPeriodDays} days`,
        },
        { label: 'Success', tooltip: 'Percentage of successful builds' },
        { label: 'YAML pipelines', tooltip: 'Pipelines configured using a YAML file' },
        {
          label: 'Uses central template',
          tooltip: 'Pipelines using the standard template',
        },
      ],
      rows: groups.map(group => {
        ...
        return {
          ...
          values: [
            ...
            },
            {
              ...
              content: reposMetric(
                <LabelWithSparkline
                  label={divide(repoStats.builds.successful, repoStats.builds.total)
                    .map(toPercentage)
                    .getOr('-')}
            ...
            {
              value: divide(
                repoStats.ymlPipelines.count,
                repoStats.ymlPipelines.total
              ).getOr(0),
              content: reposMetric(
                repoStats.ymlPipelines.total === 0
                  ? '-'
                  : `${Math.round(
                      (repoStats.ymlPipelines.count * 100) / repoStats.ymlPipelines.total
                    )}%`
              ),
            },
            {
              value: divide(
                repoStats.usesCentralTemplate.count,
                repoStats.usesCentralTemplate.total
              ).getOr(0),
              content: reposMetric(
                repoStats.usesCentralTemplate.total === 0
                  ? '0%'
                  : `${Math.round(
                      (repoStats.usesCentralTemplate.count * 100) /
                        repoStats.usesCentralTemplate.total
                    )}%`
              ),
            },
  ...
  return <CollapsibleSection heading="CI Builds" table={table} />;
};
...
const Releases: React.FC<{ groups: SummaryMetrics['groups'] }> = ({ groups }) => {
  const table: CollapsibleSectionProps['table'] = useCallback(
    () => ({
      columns: [
        null,
        {
          label: 'Master-only releases',
          tooltip:
            'Percentage of releases where all the artifacts were from the master branch',
        },
        {
          label: 'Starts with artifact',
          tooltip: 'Number of release pipelines that start with an artifact',
        },
        {
          label: 'Repos with release pipelines',
          tooltip: 'Number of repos having release pipelines',
        },
      ],
      rows: groups.map(group => {
        ...
        return {
          ...
          values: [
            ...
            {
              value: divide(
                pipelineStats.startsWithArtifact,
                pipelineStats.pipelines
              ).getOr(0),
              content: pipelinesMetric(
  ...
  return <CollapsibleSection heading="Releases" table={table} />;
};
