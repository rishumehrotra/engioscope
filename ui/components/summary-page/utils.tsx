...
export const flattenSummaryGroups = (summaryGroups: SummaryGroups) => {
  ...
  const merged = Object.values(summaryGroups).reduce<IntermediateFlattenedGroups>(
    (acc, group) => ({
      velocity: acc.velocity + group.velocity,
      velocityByWeek: group.velocityByWeek.reduce((acc, velocity, index) => {
      ...
      cycleTimeByWeek: group.cycleTimeByWeek.reduce((acc, cycleTimes, index) => {
      ...
      wipTrend: group.wipTrend.reduce<number[]>((acc, item, index) => {
      ...
      leakageByWeek: group.leakageByWeek.reduce<number[]>((acc, leakage, index) => {
...
export const workItemTypeByName =
  (name: string) => (workItemTypes: Record<string, UIWorkItemType>) => {
...
export const allExceptExpectedKeys = (group: SummaryItemProps['group']) => {
  const expectedKeys: Set<SummaryGroupKey> = new Set([
    'collection',
    'groupName',
    'portfolioProject',
    'project',
...
export const increaseIsBetter = (data: number[]) => {
  ...
  return end - start > 0 ? '#009966' : end - start === 0 ? 'grey' : '#dd0000';
};
