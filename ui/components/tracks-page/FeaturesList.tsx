...
const FeaturesListInner: React.FC<{ tracks: TrackwiseData }> = ({ tracks }) => {
  ...
  const [wipHandling, setWipHandling] = useState<
    'ignore-last-state' | 'use-today-as-end-date'
  >('ignore-last-state');
              ...
              <details key={track} className="my-6">
                <summary className="cursor-pointer">
                  ...
                  <GanttBar
                    maxTime={maxTime}
                    className="ml-3 mt-1"
                    stages={Object.entries(ts).map(
                      ([label, { time, count, isWorkCenter }]) => ({
                        label,
                    ...
                    <li key={item.id} className="mt-4 mb-7">
                      ...
                      >
                        ...
                        {tracks.times[item.id].end ? (
                          <span className="text-xs uppercase bg-green-600 px-1 py-0.5 inline-block ml-1 text-white rounded-md">
                            Live
                          </span>
                        ) : (
                          <span className="text-xs uppercase bg-orange-400 px-1 py-0.5 inline-block ml-1 text-white rounded-md">
                            WIP
                          </span>
                        )}
                      </a>
                      <GanttBar
                        ...
                        stages={timeSpent[item.id]
                          ...
                          .map(({ label, start, end, isWorkCenter }) => ({
                            label,
                            time:
...
const FeaturesList: React.FC<{ tracks: TrackwiseData[] }> = ({ tracks }) => (
  <>
    {tracks.map(track => (
      <details key={`${track.collection}/${track.project}`} open={tracks.length === 1}>
