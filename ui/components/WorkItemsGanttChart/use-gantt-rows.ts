...
const isWorkItemNode = (node: TreeNode): node is WorkItemNode => node.type === 'workitem';
...
const recomputeDerivedValues = (node: ProjectNode): ProjectNode => {
  const children = node.children.map<WorkItemTypeNode>(workItemTypeNode => {
    const children = workItemTypeNode.children.map<WorkItemTypeNode['children'][number]>(
      workItemOrEnvNode => {
    ...
    return {
      ...workItemTypeNode,
      children,
      workItemIds: children.flatMap(c => (isWorkItemNode(c) ? [wid(c)] : [])),
      childCount: children.reduce(
        (acc, node) => acc + (isWorkItemEnvironmentNode(node) ? node.childCount : 1),
        0
      ),
    } as WorkItemTypeNode;
  });
...
const isInAncestors =
  (ancestors: WithoutChildren<WorkItemNode>[]) => (workItemId: number) =>
    ancestors.every(a => wid(a) !== workItemId);

const groupByWorkItemType =
  (
  ...
  (ancestors: WithoutChildren<WorkItemNode>[], workItemIds: number[] | undefined) =>
    (workItemIds || [])
      ...
      .map(workItemById)
      .reduce<Record<string, UIWorkItem[]>>(
        (acc, workItem) => ({
          ...acc,
...
const constructTree = (
  ...
  const buildForAncestor = (
    ancestors: WithoutChildren<WorkItemNode>[],
    currentDepth = 0
    ...
    const groupedByWorkItemType = byWorkItemType(ancestors, childIdsOf(wid(parent)));

    const uiElementsByWorkItemType = Object.entries(groupedByWorkItemType).reduce<
      Record<string, Pick<WorkItemTypeNode, 'color' | 'icon'>>
    >(
      (acc, [wit, workItems]) => ({
        ...acc,
        [wit]: {
    ...
    return Object.entries(groupedByWorkItemType).map<WorkItemTypeNode>(
      ([wit, workItems]) => {
        const groupedByEnvironment = workItems.reduce<Record<string, UIWorkItem[]>>(
          (acc, workItem) => ({
            ...acc,
            [workItem.env || 'no-environment']: [
              ...(acc[workItem.env || 'no-environment'] || []),
              workItem,
            ],
          }),
          {}
        ...
        const nodesByEnvironment = Object.entries(
          ...
          const children = workItems
            ...
            .map<WorkItemNode>(workItem => {
              const workItemNode: WithoutChildren<WorkItemNode> = {
                type: 'workitem',
                workItem,
                path: `${parent.path}/${wit}/${environmentName}/${workItem.id}`.replace(
                  'no-environment/',
                  ''
                ),
              ...
              return {
                ...workItemNode,
                expandedState:
                  (childIdsOf(workItem.id) || []).length === 0
                    ? 'no-children'
                    : 'collapsed',
                children: buildForAncestor(
                  [...ancestors, workItemNode],
                  currentDepth + 1
                ),
              };
            });
          ...
          return {
            ...
            minTimestamp: Math.min(
              ...workItems.map(wi => new Date(wi.created.on).getTime())
            ),
            maxTimestamp: Math.max(
              ...workItems.map(wi => new Date(wi.updated.on).getTime())
            ),
        ...
        const children: (WorkItemEnvironmentNode | WorkItemNode)[] =
          envs.length === 1 && envs[0] === 'no-environment'
            ? nodesByEnvironment[0].children.map<WorkItemNode>(c => ({
                ...c,
                depth: c.depth - 1,
              }))
            : nodesByEnvironment;
        ...
        return {
          ...
          childCount: Object.values(nodesByEnvironment).reduce(
            (acc, n) => acc + n.childCount,
            0
          ),
          minTimestamp: Math.min(
            ...workItems.map(wi => new Date(wi.created.on).getTime())
  ...
  return (workItemId: number, projectName: string) => {
    ...
    const ownProjectChildren = fullTree
      .map(
        filterTree<TreeNode>(
          node => isWorkItemNode(node) && node.workItem.project === projectName
        )
      )
      .filter(exists)
      .map(
    ...
    const allProjectsNode: ProjectNode = {
      ...
      depth: 0,
      expandedState: 'collapsed',
      children: fullTree.map(
        mapTree<TreeNode, TreeNode>(node => ({
          ...node,
          path: `/all-projects${node.path}`,
        }))
      ) as WorkItemTypeNode[],
...
const toggleExpandState =
  (path: string) =>
  (node: TreeNode | ProjectNode): TreeNode | ProjectNode => {
    if (node.path === path) {
      if (node.expandedState === 'no-children') return node;

      return {
        ...node,
        expandedState: node.expandedState === 'collapsed' ? 'expanded' : 'collapsed',
      };
    }
...
export const isWorkItemRow = (
  row: Row
): row is Omit<WorkItemNode, 'children'> & { childCount: number } =>
  row.type === 'workitem';

export const isWorkItemTypeRow = (
  row: Row
): row is Omit<WorkItemTypeNode, 'children'> & { childCount: number } =>
  row.type === 'workitem-type';

export const isWorkItemEnvironmentRow = (
  row: Row
): row is Omit<WorkItemEnvironmentNode, 'children'> & { childCount: number } =>
  row.type === 'workitem-environment';

export const isProjectRow = (
  row: Row
): row is Omit<ProjectNode, 'childrne'> & { childCount: number } =>
  row.type === 'project';
...
const useGanttRows = (
  ...
  useEffect(() => {
    ...
    setTree(
      constructTree(workItemsIdTree, workItemsById, workItemType)(workItemId, projectName)
    );
