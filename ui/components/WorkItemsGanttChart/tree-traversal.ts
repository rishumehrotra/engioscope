...
export const reduceTree = <T, U>(reducer: (acc: U, node: Tree<T>) => U) => (init: U, tree: Tree<T>): U => {
  const acc = reducer(init, tree);
  if (!tree.children) return acc;
  return tree.children.reduce(reduceTree(reducer), acc);
};

export const mapTree = <T, U>(mapper: (node: Tree<T>) => Tree<U>) => (tree: Tree<T>): Tree<U> => {
 ...
 */
export const filterTree = <T>(predicate: (node: Tree<T>) => boolean) => (tree: Tree<T>): Tree<T> | null => {
