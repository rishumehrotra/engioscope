...
const createChangeProgramTask =
  (collectionConfig: ParsedCollection) =>
  (wi: WorkItem): UIChangeProgramTask => ({
    id: wi.id,
...
const changeProgramTasks = async (
  collectionConfig: ParsedCollection,
  getCollectionWorkItemIdsForQuery: ReturnType<
    typeof azure
  ...
  const wis = getCollectionWorkItems(
    collectionConfig.name,
    queryResult.workItems.map(prop('id')),
    queryName
  );
...
export default (
  ...
  async (collectionConfig: ParsedCollection) => {
    if (!collectionConfig.changeProgram) return [];
    return changeProgramTasks(
      collectionConfig,
      getCollectionWorkItemIdsForQuery,
