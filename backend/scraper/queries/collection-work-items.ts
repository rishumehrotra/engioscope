...
const skipChildren = (collectionConfig: ParsedCollection) =>
  collectionConfig.workitems.skipChildren?.length
    ? `AND NOT [Target].[System.WorkItemType] IN (${join(
        collectionConfig.workitems.skipChildren
      )}) `
    : '';
...
const ignoreStates = (collectionConfig: ParsedCollection) =>
  collectionConfig.workitems.ignoreStates?.length
    ? `
      AND NOT [Source].[System.State] IN (${join(
        collectionConfig.workitems.ignoreStates
      )})
      AND NOT [Target].[System.State] IN (${join(
        collectionConfig.workitems.ignoreStates
      )}) 
    `
    : '';
...
const additionalClausesForWIPWorkItems = (collectionConfig: ParsedCollection) => {
  const clause = (collectionConfig.workitems.types || [])
    ...
    .map(
      ...
      AND (${type.startDate
        .map(startDate => `[Source].[${startDate}] <> ''`)
        .join(' OR ')})
...
export const queryForCollectionWorkItems = (
  queryFrom: Date,
  ...
  return `
    ...
    WHERE
      ...
      AND [Source].[System.TeamProject] IN (${join(
        collectionConfig.projects.map(p => p.name)
      )})
      AND [Target].[System.TeamProject] IN (${join(
        collectionConfig.projects.map(p => p.name)
      )})
