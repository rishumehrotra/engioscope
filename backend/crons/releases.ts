...
import { getLastReleaseFetchDate, setLastReleaseFetchDate } from '../models/cron-update-dates.js';
...
export const getReleases = async () => {
  ...
  await Promise.all(collectionsAndProjects()
    .map(async ([collection, project]) => {
      await getReleasesAsChunks(
        collection.name,
        project.name,
        await getLastReleaseFetchDate(collection.name, project.name) || defaultQueryStart(),
...
export const getReleaseUpdates = async () => {
  ...
  await Promise.all(collectionsAndProjects()
    .map(async ([collection, project]) => {
      const releaseUpdateDates = await getReleaseUpdateDates(collection.name, project.name);
